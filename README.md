> [!IMPORTANT]
> ## 🔧 Fork Notice — Claude Code CLI Provider Added
>
> This is a modified version of the original [be-more-agent](https://github.com/brenpoly/be-more-agent) project.
> The core behaviour is unchanged. The following additions were made to allow the agent to use
> the **Claude Code CLI** (`claude`) as an alternative LLM backend alongside the default Ollama integration.
>
> **Summary of changes (`agent.py`, `config.json`):**
>
> - Added a `"llm_provider"` key to `config.json` and `DEFAULT_CONFIG` — set to `"ollama"` (default) or `"claude-cli"`.
> - Introduced an `OllamaProvider` class that wraps all existing `ollama.*` calls unchanged.
> - Introduced a `ClaudeCLIProvider` class that calls the local `claude` binary as a subprocess,
>   streaming output via `--output-format stream-json`. Authenticates through `claude login` — no `ANTHROPIC_API_KEY` needed.
>   Falls back to Ollama automatically if the `claude` binary is not found on `$PATH`.
> - Added a `make_llm_provider(config)` factory that reads `llm_provider` from config and returns the right instance.
> - Replaced the 4 direct `ollama.*` call sites in `agent.py` with `LLM.warmup()`, `LLM.shutdown()`,
>   `LLM.chat_stream()`, and `LLM.chat()` so both providers share the same code path.
